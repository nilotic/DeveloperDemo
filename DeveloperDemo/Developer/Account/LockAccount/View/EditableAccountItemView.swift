...
struct EditableAccountItemView: View {
    
    // MARK: - Value
    // MARK: Public
    let data: EditableAccountItem
    ...
    private var nameView: some View {
        Text(String(localizedFormat: "accountName", data.name))
    ...
    private var userNumberView: some View {
        Text(String(data.userNumber))
            .font(.system(size: 15))
            .foregroundColor(data.status == .locked ? .pink : .indigo)
    }
}
...
#Preview {
    List {
        EditableAccountItemView(data: .placeholder)
    }
}
#endif
